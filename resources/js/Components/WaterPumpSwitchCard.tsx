...
const WaterPumpSwitchCard: React.FC<Props> = ({ isEnabled }) => {
  ...
  const togglePump = async () => {
    ...
    try {
      const response = await axios.post('/api/control-pump', {
        water_pump: !isOn, // kirim status berlawanan untuk toggle
      });

      console.log('✅ Pompa berhasil dikontrol:', response.data);
      setIsOn((prev) => !prev);
    } catch (error: any) {
      console.error('❌ Gagal mengontrol pompa:', error?.response?.data || error.message);
      alert('Terjadi kesalahan saat mengontrol pompa.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="bg-white rounded-xl shadow p-4 w-full h-full flex flex-col justify-between">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-gray-800">Pompa Air</h2>
...
export default WaterPumpSwitchCard;
